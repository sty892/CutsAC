...
public class ProxyAlertMessenger extends PacketListenerAbstract {
    ...
    public ProxyAlertMessenger() {
        ...
        if (usingProxy) {
            ...
            CustACAPI.INSTANCE.getPlatformServer().registerOutgoingPluginChannel("BungeeCord");
        }
    }
    ...
    public static void sendPluginMessage(String message) {
        ...
        Iterables.getFirst(CustACAPI.INSTANCE.getPlatformPlayerFactory().getOnlinePlayers(), null).sendPluginMessage("BungeeCord", out.toByteArray());
    }

    public static boolean canSendAlerts() {
        return usingProxy && CustACAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("alerts.proxy.send", false) && !CustACAPI.INSTANCE.getPlatformPlayerFactory().getOnlinePlayers().isEmpty();
    }

    public static boolean canReceiveAlerts() {
        return usingProxy && CustACAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("alerts.proxy.receive", false) && CustACAPI.INSTANCE.getAlertManager().hasAlertListeners();
    }
