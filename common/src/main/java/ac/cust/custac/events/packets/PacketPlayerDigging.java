...
import ac.cust.custac.utils.item.ItemBehaviour;
import ac.cust.custac.utils.item.ItemBehaviourRegistry;
...
public class PacketPlayerDigging extends PacketListenerAbstract {
    ...
    public static void handleUseItem(@NotNull CustACPlayer player, @NotNull InteractionHand hand) {
    ...
    public void onPacketReceive(PacketReceiveEvent event) {
        if (event.getPacketType() == PacketType.Play.Client.PLAYER_DIGGING) {
            ...
            if (dig.getAction() == DiggingAction.RELEASE_USE_ITEM) {
                final CustACPlayer player = CustACAPI.INSTANCE.getPlayerDataManager().getPlayer(event.getUser());
        ...
        if (event.getPacketType() == PacketType.Play.Client.HELD_ITEM_CHANGE) {
            ...
            if (player.packetStateData.lastSlotSelected != slot) {
                if (player.isResetItemUsageOnSlotChange() && CustACAPI.INSTANCE.getItemResetHandler().getItemUsageHand(player.platformPlayer) == InteractionHand.MAIN_HAND) {
