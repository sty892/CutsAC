...
import ac.cust.custac.utils.data.TrackerData;
import ac.cust.custac.utils.data.packetentity.DashableEntity;
import ac.cust.custac.utils.data.packetentity.PacketEntity;
import ac.cust.custac.utils.data.packetentity.PacketEntityHook;
...
public class PacketEntityReplication extends Check implements PacketCheck {
    ...
    public PacketEntityReplication(CustACPlayer player) {
        super(player);
    }
    ...
    public void onPacketSend(PacketSendEvent event) {
        // ensure custac is the one that sent the transaction
            ...
            if (slot.getWindowId() == 0) {
                Runnable task = () -> {
                    if (slot.getSlot() - 36 == player.packetStateData.lastSlotSelected && (
                        ...
                        if (player.isResetItemUsageOnItemUpdate() && hand == CustACAPI.INSTANCE.getItemResetHandler().getItemUsageHand(player.platformPlayer)) {
                            CustACAPI.INSTANCE.getItemResetHandler().resetItemUsage(player.platformPlayer);
                        }
                    }
                };
                ...
                Runnable task = () -> {
                    if (player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_8)) {
                        ...
                        if (items.getItems().size() > 45 && !player.inventory.getOffHand().is(items.getItems().get(45).getType())) {
                            ...
                            if (player.isResetItemUsageOnItemUpdate() && CustACAPI.INSTANCE.getItemResetHandler().getItemUsageHand(player.platformPlayer) == InteractionHand.OFF_HAND) {
                                CustACAPI.INSTANCE.getItemResetHandler().resetItemUsage(player.platformPlayer);
                            }
                        }
                        ...
                        if (!player.inventory.getHeldItem().is(items.getItems().get(player.packetStateData.lastSlotSelected + 36).getType())) {
                            ...
                            if (player.isResetItemUsageOnItemUpdate() && CustACAPI.INSTANCE.getItemResetHandler().getItemUsageHand(player.platformPlayer) == InteractionHand.MAIN_HAND) {
                                CustACAPI.INSTANCE.getItemResetHandler().resetItemUsage(player.platformPlayer);
                            }
                        }
                    }
        ...
        // 1.8 clients fail to send the RELEASE_USE_ITEM packet when a window is opened client sided while using an item
            ...
            if (!attach.isLeash()) {
                // Alright, let's convert this to the 1.9+ format to make it easier for custac
    ...
    private void handleMoveEntity(PacketSendEvent event, int entityId, double deltaX, double deltaY, double deltaZ, Float yaw, Float pitch, boolean isRelative, boolean hasPos) {
        ...
        if (data != null) {
            ...
            if (isRelative) {
                ...
                // This causes impossible hits, so custac must replace this with a teleport entity packet
    ...
    private boolean isDirectlyAffectingPlayer(CustACPlayer player, int entityID) {
        // The attributes for this entity is active, currently
