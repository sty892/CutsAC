...
import ac.cust.custac.utils.data.RotationData;
...
public class PacketServerTeleport extends PacketListenerAbstract {
    ...
    public void onPacketSend(PacketSendEvent event) {
        ...
        if (event.getPacketType() == PacketType.Play.Server.PLAYER_ROTATION) {
            ...
            player.sendTransaction();
            player.pendingRotations.add(new RotationData(packet.getYaw(), CustACMath.clamp(packet.getPitch() % 360F, -90F, 90F), player.getLastTransactionSent()));
            event.getTasksAfterSend().add(player::sendTransaction);
        }
