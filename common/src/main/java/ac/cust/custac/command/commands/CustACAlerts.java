...
public class CustACAlerts implements BuildableCommand {
    @Override
    ...
    private void handleAlerts(@NotNull CommandContext<Sender> context) {
        ...
        if (sender.isPlayer()) {
            CustACAPI.INSTANCE.getAlertManager().toggleAlerts(Objects.requireNonNull(context.sender().getPlatformPlayer()), false);
