...
public class CustACVersion implements BuildableCommand {
    ...
    public static void checkForUpdatesAsync(Sender sender) {
        String current = CustACAPI.INSTANCE.getExternalAPI().getCustACVersion();
        sender.sendMessage(Component.text()
                .append(Component.text("CustAC Version: ").color(NamedTextColor.GRAY))
        ...
        lastCheck = now;
        CustACAPI.INSTANCE.getScheduler().getAsyncScheduler().runNow(CustACAPI.INSTANCE.getGrimPlugin(), () -> checkForUpdates(sender));
    }
    ...
    private static void checkForUpdates(Sender sender) {
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(CommonCustACArguments.API_URL.value() + "updates"))
                    .GET()
                    .header("User-Agent", "CustACAC/" + CustACAPI.INSTANCE.getExternalAPI().getCustACVersion())
            ...
            if (statusCode < 200 || statusCode >= 300) {
                ...
                sender.sendMessage(Objects.requireNonNullElseGet(msg, () -> Component.text()
                        ...
                        .append(Component.text(" Failed to check latest CustACAC version. Update server responded with status code: ")
            ...
            if (object.has("status")) {
                ...
                status = Status.SemVer.getVersionStatus(CustACAPI.INSTANCE.getExternalAPI().getCustACVersion(), latest);
            }
            //
            Component msg = switch (status) {
                case AHEAD ->
                        Component.text("You are using a development version of CustACAC").color(NamedTextColor.LIGHT_PURPLE);
                case UPDATED ->
                        Component.text("You are using the latest version of CustACAC").color(NamedTextColor.GREEN);
                case OUTDATED -> Component.text()
                        .append(Component.text("New CustACAC version found!").color(NamedTextColor.AQUA))
                        ...
                        .build();
                case UNKNOWN ->
                        Component.text("You are using an unknown CustACAC version.").color(NamedTextColor.RED);
            };
            ...
            LogUtil.error("Failed to check latest CustACAC version.", e);
        }
    }
