...
import ac.cust.custac.command.CommandUtils;
...
public class CustACList implements BuildableCommand {
    ...
    private void handleListPlayers(Sender sender) {
        ...
        final Map<UUID, PlatformPlayer> onlinePlayers = CustACAPI.INSTANCE.getPlatformPlayerFactory().getOnlinePlayers()
        ...
        //
        for (CustACPlayer entry : CustACAPI.INSTANCE.getPlayerDataManager().getEntries()) {
            if (after) {
            ...
            boolean exempt = !CustACAPI.INSTANCE.getPlayerDataManager().shouldCheck(entry.user);
