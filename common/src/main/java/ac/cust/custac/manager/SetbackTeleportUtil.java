...
import ac.cust.custac.checks.impl.badpackets.BadPacketsN;
...
import ac.cust.custac.utils.data.TeleportAcceptData;
import ac.cust.custac.utils.data.TeleportData;
import ac.cust.custac.utils.data.VectorData;
import ac.cust.custac.utils.data.VelocityData;
...
public class SetbackTeleportUtil extends Check implements PostPredictionCheck {
    ...
    public SetbackTeleportUtil(CustACPlayer player) {
        super(player);
    }
    ...
    public void executeForceResync() {
        if (player.gamemode == GameMode.SPECTATOR || player.disableCustAC)
    ...
    private boolean isExempt() {
        ...
        if (player.disableCustAC) return true;
        ...
        try {
            ...
            if (player.inVehicle()) {
                ...
                if (player.compensatedEntities.serverPlayerVehicle != null) {
                    ...
                    CustACAPI.INSTANCE.getScheduler().getEntityScheduler().execute(player.platformPlayer, CustACAPI.INSTANCE.getGrimPlugin(), () -> {
                        if (player.platformPlayer != null) {
                            CustACEntity vehicle = player.platformPlayer.getVehicle();
                            if (vehicle != null) {
                                vehicle.eject();
                            }
                        }
            ...
            if (PacketEvents.getAPI().getServerManager().getVersion().isOlderThanOrEquals(ServerVersion.V_1_7_10)) {
                y += 1.62; // 1.7 teleport offset if custac ever supports 1.7 again
            }
    ...
    public boolean insideUnloadedChunk() {
        Column column = player.compensatedWorld.getChunk(CustACMath.floor(player.x) >> 4, CustACMath.floor(player.z) >> 4);

        // If true, the player is in an unloaded chunk
        return !player.disableCustAC && (column == null || column.transaction() >= player.lastTransactionReceived.get() ||
