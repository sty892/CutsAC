...
import ac.cust.custac.utils.data.webhook.discord.Embed;
import ac.cust.custac.utils.data.webhook.discord.EmbedField;
import ac.cust.custac.utils.data.webhook.discord.EmbedFooter;
...
public class DiscordManager implements StartableInitable, ReloadableInitable {
    ...
    private static String validatedConfigURL(String configPath, String defaultURL) {
        String url = CustACAPI.INSTANCE.getConfigManager().getConfig().getStringElse("embed-image-url", defaultURL);
    ...
    public void reload() {
        try {
            ...
            ConfigManager config = CustACAPI.INSTANCE.getConfigManager().getConfig();
            ...
            embedFooterUrl = validatedConfigURL("embed-footer-url", "https://custac.ac/images/custac.png");
            embedFooterText = config.getStringElse("embed-footer-text", "v%custac_version%");
            embedTitle = config.getStringElse("embed-title", "**CustAC Alert**");
    ...
    public void sendAlert(@NotNull CustACPlayer player, String verbose, String checkName, int violations) {
        if (isDisabled()) {
            return;
        }

        // Per-alert overlay — avoids polluting the global static map
        Map<String, String> statics = new HashMap<>(CustACAPI.INSTANCE.getExternalAPI().getStaticReplacements());
        statics.put("%check%", checkName);
        statics.put("%violations%", Integer.toString(violations));

        Map<String, Function<GrimUser, String>> dynamics = CustACAPI.INSTANCE.getExternalAPI().getVariableReplacements();
    ...
    public CompletableFuture<Boolean> sendWebhookMessage(WebhookMessage message) {
        ...
        if (!taskStarted.getAndSet(true)) {
            ...
            CustACAPI.INSTANCE.getScheduler().getAsyncScheduler().runAtFixedRate(CustACAPI.INSTANCE.getGrimPlugin(), DiscordManager::tick, 0, 1);
        }
