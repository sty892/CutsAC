package ac.cust.custac.manager.config;
...
public class ConfigManagerFileImpl implements ConfigManager, BasicReloadable {
    ...
    private File getConfigFile(String path) {
        return new File(CustACAPI.INSTANCE.getGrimPlugin().getDataFolder(), path);
    }

    @Override
    public void reload() {
        CustACAPI.INSTANCE.getGrimPlugin().getDataFolder().mkdirs();
        if (!initialized) {
            initialized = true;
            config.addSource(CustACAPI.class, "config", getConfigFile("config.yml"));
            config.addSource(CustACAPI.class, "messages", getConfigFile("messages.yml"));
            config.addSource(CustACAPI.class, "discord", getConfigFile("discord.yml"));
            config.addSource(CustACAPI.class, "punishments", getConfigFile("punishments.yml"));
        }
    ...
    private void upgrade() {
        File config = new File(CustACAPI.INSTANCE.getGrimPlugin().getDataFolder(), "config.yml");
        if (config.exists()) {
            try {
    ...
    private void removeLegacyTwoPointOne(File config) throws IOException {
        ...
        Files.move(config.toPath(), new File(CustACAPI.INSTANCE.getGrimPlugin().getDataFolder(), "config-2.1.old.yml").toPath());
    }
    ...
    private void addMissingPunishments() {
        File config = new File(CustACAPI.INSTANCE.getGrimPlugin().getDataFolder(), "punishments.yml");
        String configString;
        if (config.exists()) {
            try {
    ...
    private void fixBadPacketsAndAdjustPingConfig(File config, String configString) {
        ...
        File punishConfig = new File(CustACAPI.INSTANCE.getGrimPlugin().getDataFolder(), "punishments.yml");
    ...
    private void newOffsetNewDiscordConf(File config, String configString) throws IOException {
        ...
        File discordFile = new File(CustACAPI.INSTANCE.getGrimPlugin().getDataFolder(), "discord.yml");
    ...
    private void addSuperDebug(File config, String configString) throws IOException {
        ...
        File messageFile = new File(CustACAPI.INSTANCE.getGrimPlugin().getDataFolder(), "messages.yml");
    ...
    private void removeAlertsOnJoin(File config, String configString) throws IOException {
        configString = configString.replaceAll("  # Should players with custac\\.alerts permission automatically enable alerts on join\\?\r?\n  enable-on-join: (?:true|false)\r?\n", ""); // en
    ...
    private void addPacketSpamThreshold(File config, String configString) throws IOException {
        configString += "\n# CustAC sometimes cancels illegal packets such as with timer, after X packets in a second cancelled, when should\n" +
                "# we simply kick the player? This is required as some packet limiters don't count packets cancelled by custac.\n" +
