...
import ac.cust.custac.manager.init.ReloadableInitable;
import ac.cust.custac.manager.init.start.StartableInitable;
import ac.cust.custac.manager.violationdatabase.mysql.MySQLViolationDatabase;
import ac.cust.custac.manager.violationdatabase.postgresql.PostgresqlViolationDatabase;
import ac.cust.custac.manager.violationdatabase.sqlite.SQLiteViolationDatabase;
...
public class ViolationDatabaseManager implements StartableInitable, ReloadableInitable {
    ...
    public void load() {
        ConfigManager cfg = CustACAPI.INSTANCE.getConfigManager().getConfig();
        ...
        switch (rawType) {
            case "SQLITE" -> {
                if (!(database instanceof SQLiteViolationDatabase)) {
                    ...
                    try {
                        ...
                        LogUtil.error(
                                """
                                        IMPORTANT: Could not load SQLite driver for /custac history database.
                                        Download the minecraft-sqlite-jdbc mod/plugin for SQLite support, or change history.database.type
                                        Alternatively set history.enabled=false to remove this message if /custac history support is not desired"""
                        );
                        this.database = NoOpViolationDatabase.INSTANCE;
                        loaded = false;
            ...
            case "MYSQL" -> {
                ...
                String db = cfg.getStringElse("history.database.database", "custacac");
    ...
    public void logAlert(CustACPlayer player, String verbose, String checkName, int vls) {
        String custacVersion = CustACAPI.INSTANCE.getExternalAPI().getCustACVersion();
        CustACAPI.INSTANCE.getScheduler().getAsyncScheduler().runNow(plugin, () -> {
            database.logAlert(player, custacVersion, verbose, checkName, vls);
            fileLogger.log(player, custacVersion, verbose, checkName, vls);
        });
    }

    public int getLogCount(UUID player) {
        if (!enabled || !loaded) {
            return fileLogger.getLogCount(player);
        }
    ...
    public List<Violation> getViolations(UUID player, int page, int limit) {
        if (!enabled || !loaded) {
            return fileLogger.getViolations(player, page, limit);
        }
