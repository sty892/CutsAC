...
public final class AlertManagerImpl implements AlertManager, ConfigReloadable, StartableInitable {
    ...
    @Override
    public void start() {
        platformServer = CustACAPI.INSTANCE.getPlatformServer();
     ...
     * @throws IllegalArgumentException if the user is not a CustACPlayer.
     * @throws NullPointerException if the CustACPlayer's platformPlayer is null.
     */
    private @NotNull PlatformPlayer requirePlatformPlayerFromUser(@NotNull GrimUser user) {
        ...
        if (!(user instanceof CustACPlayer custacPlayer)) {
            // Throw a specific exception if the type is wrong
            throw new IllegalArgumentException("AlertManager action called with non-CustACPlayer user: " + user.getName());
        }

        PlatformPlayer platformPlayer = custacPlayer.platformPlayer;
    ...
    public boolean hasBrandsEnabled(@NotNull GrimUser player) {
        CustACPlayer custacPlayer = (CustACPlayer) player;
        ...
        if (custacPlayer.platformPlayer == null) return false;

        return hasBrandsEnabled(custacPlayer.platformPlayer);
    }
    ...
    public boolean hasAlertsEnabled(Player player) {
        ...
        return hasAlertsEnabled(CustACAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }

    @Override
    public void toggleAlerts(Player player) {
        if (player == null) return;
        toggleAlerts(CustACAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }

    @Override
    public boolean hasVerboseEnabled(Player player) {
        if (player == null) return false;
        return hasVerboseEnabled(CustACAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }

    @Override
    public void toggleVerbose(Player player) {
        if (player == null) return;
        toggleVerbose(CustACAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }
