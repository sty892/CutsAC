...
public class EntityFluidInteraction {
    ...
    public void update(final CustACPlayer player, final boolean ignoreCurrent) {
        ...
        int minX = CustACMath.floor(aabb.minX);
        int minY = CustACMath.floor(aabb.minY);
        int minZ = CustACMath.floor(aabb.minZ);
        int maxX = CustACMath.ceil(aabb.maxX) - 1;
        int maxY = CustACMath.ceil(aabb.maxY) - 1;
        int maxZ = CustACMath.ceil(aabb.maxZ) - 1;
        ...
        int playerX = CustACMath.floor(player.lastX);
        double playerEyeY = player.lastY + player.getEyeHeight() - 0.1111111119389534D;
        int playerZ = CustACMath.floor(player.lastZ);
    ...
    public void applyCurrentTo(final FluidTag fluid, final CustACPlayer entity, final double scale) {
    ...
    private static class Tracker {
        ...
        public void applyCurrentTo(final CustACPlayer player, final double scale) {
