...
import ac.cust.custac.checks.impl.vehicle.VehicleC;
import ac.cust.custac.checks.type.PositionCheck;
import ac.cust.custac.manager.SetbackTeleportUtil;
import ac.cust.custac.player.CustACPlayer;
import ac.cust.custac.predictionengine.movementtick.MovementTickerCamel;
import ac.cust.custac.predictionengine.movementtick.MovementTickerHappyGhast;
import ac.cust.custac.predictionengine.movementtick.MovementTickerHorse;
import ac.cust.custac.predictionengine.movementtick.MovementTickerNautilus;
import ac.cust.custac.predictionengine.movementtick.MovementTickerPig;
import ac.cust.custac.predictionengine.movementtick.MovementTickerPlayer;
import ac.cust.custac.predictionengine.movementtick.MovementTickerStrider;
import ac.cust.custac.predictionengine.predictions.PredictionEngineNormal;
import ac.cust.custac.predictionengine.predictions.rideable.PredictionEngineBoat;
import ac.cust.custac.predictionengine.predictions.rideable.PredictionEngineRideableUtils;
...
import ac.cust.custac.utils.data.SetBackData;
...
import ac.cust.custac.utils.data.packetentity.PacketEntityTrackXRot;
...
import ac.cust.custac.utils.nmsutil.BoundingBoxSize;
...
public class MovementCheckRunner extends Check implements PositionCheck {
    ...
    public MovementCheckRunner(CustACPlayer player) {
        super(player);
    }
    ...
    private void handleTeleport(PositionUpdate update) {
        ...
        //
        // However, for some reason, this behaviour does not work correctly in CustAC
    ...
    private void check(PositionUpdate update) {
        ...
        if (!player.isFlying && player.isSneaking && Collisions.isAboveGround(player)) {
            ...
            double posX = Math.max(0.05, CustACMath.clamp(player.actualMovement.getX(), -16, 16) + 0.05);
            double posZ = Math.max(0.05, CustACMath.clamp(player.actualMovement.getZ(), -16, 16) + 0.05);
            double negX = Math.min(-0.05, CustACMath.clamp(player.actualMovement.getX(), -16, 16) - 0.05);
            double negZ = Math.min(-0.05, CustACMath.clamp(player.actualMovement.getZ(), -16, 16) - 0.05);
        ...
        if (player.vehicleData.wasVehicleSwitch || player.vehicleData.lastDummy) {
            ...
            if (riding != null) {
                ...
                // their similar code may also work on custac.
                //
...
//                    player.uncertaintyHandler.isSteppingOnSlime = true;
        ...
        } else if (player.disableCustAC || (PacketEvents.getAPI().getServerManager().getVersion().isNewerThanOrEquals(ServerVersion.V_1_8) && player.gamemode == GameMode.SPECTATOR) || player.isFlying || (player.isExemptElytra() && player.isGliding)) {
