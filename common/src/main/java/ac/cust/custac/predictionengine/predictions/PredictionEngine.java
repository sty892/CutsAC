...
import ac.cust.custac.predictionengine.movementtick.MovementTickerPlayer;
...
import ac.cust.custac.utils.math.Vec2;
...
import ac.cust.custac.utils.nmsutil.JumpPower;
import ac.cust.custac.utils.nmsutil.Riptide;
...
public class PredictionEngine {
    ...
    public static Vector3dm clampMovementToHardBorder(CustACPlayer player, Vector3dm outputVel) {
        // TODO: Reimplement
        return outputVel;
    }

    public static Vector3dm transformInputsToVector(CustACPlayer player, Vector3dm theoreticalInput) {
    ...
    public static Vec2 modifyInput(CustACPlayer player, Vec2 moveVector) {
    ...
    private static float distanceToUnitSquare(Vec2 input) {
        ...
        return CustACMath.sqrt(1.0F + CustACMath.square(additional));
    }
    ...
    private static float getItemUseSpeedMultiplier(CustACPlayer player) {
    ...
    public void guessBestMovement(float speed, CustACPlayer player) {
    ...
    private void doPredictions(CustACPlayer player, List<VectorData> possibleVelocities, float speed) {
    ...
    private Pair<Vector3dm, Vector3dm> doSeekingWallCollisions(CustACPlayer player, Vector3dm primaryPushMovement, Vector3dm originalClientVel, VectorData clientVelAfterInput) {
    ...
    private void addZeroPointThreeToPossibilities(float speed, CustACPlayer player, List<VectorData> possibleVelocities) {
    ...
    public void addFluidPushingToStartingVectors(CustACPlayer player, Set<VectorData> data) {
    ...
    public Set<VectorData> fetchPossibleStartTickVectors(CustACPlayer player) {
        ...
        // Inputs are done AFTER fluid pushing, https://github.com/MWHunter/CustAC/issues/660
    ...
    private void addNonEffectiveAI(CustACPlayer player, Set<VectorData> data) {
    ...
    private void addAttackSlowToPossibilities(CustACPlayer player, Set<VectorData> velocities) {
    ...
    public void applyMovementThreshold(CustACPlayer player, Set<VectorData> velocities) {
    ...
    public void addExplosionToPossibilities(CustACPlayer player, Set<VectorData> existingVelocities) {
    ...
    public int sortVectorData(VectorData a, VectorData b, CustACPlayer player) {
        int aScore = 0;
        int bScore = 0;
    ...
    public Vector3dm handleStartingVelocityUncertainty(CustACPlayer player, VectorData vector, Vector3dm targetVec) {
        ...
        // https://github.com/MWHunter/CustAC/issues/398
        ...
        //
        //
        // CustAC's old solution with hard lerping entities was to just give a ton of direct offset reduction
    ...
    public void endOfTick(CustACPlayer player, double d) {
        player.canSwimHop = canSwimHop(player);
        player.lastWasClimbing = 0;
    }

    private void loopVectors(CustACPlayer player, Set<VectorData> possibleVectors, float speed, List<VectorData> returnVectors) {
        // Stop omni-sprint
    ...
    public boolean canSwimHop(CustACPlayer player) {
    ...
    public Vector3dm getMovementResultFromInput(CustACPlayer player, Vector3dm inputVector, float f, float f2) {
        float f2InRadians = CustACMath.radians(f2);
    ...
    public Vector3dm handleOnClimbable(Vector3dm vector, CustACPlayer player) {
        return vector;
    }

    public void doJump(CustACPlayer player, Vector3dm vector) {
