    ...
    private static boolean forEachBlockIntersectedBetween(Vector3d start, Vector3d end, SimpleCollisionBox boundingBox, BlockStepVisitor blockStepVisitor) {
        ...
        if (direction.lengthSquared() < CustACMath.square(0.99999F)) {
            ...
            if (iterationCount < 0) {
                ...
                for (Vector3i blockPos : SimpleCollisionBox.betweenClosed(boundingBox)) {
                    if (!alreadyVisited.contains(CustACMath.asLong(blockPos)) && !blockStepVisitor.visit(blockPos, iterationCount + 1)) {
                        return false;
                    }
                }
                ...
                for (int x = currentX; x <= endX; x++) {
                    for (int y = currentY; y <= endY; y++) {
                        for (int z = currentZ; z <= endZ; z++) {
                            if (alreadyVisited.add(CustACMath.asLong(x, y, z)) && !blockStepVisitor.visit(new Vector3i(x, y, z), iterationCount)) {
                                return -1;
                            }
                        }
                    }
