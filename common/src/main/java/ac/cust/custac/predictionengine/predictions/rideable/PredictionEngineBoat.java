...
import ac.cust.custac.utils.enums.BoatEntityStatus;
...
public class PredictionEngineBoat extends PredictionEngine {
    public PredictionEngineBoat(CustACPlayer player) {
    ...
    private static BoatEntityStatus getStatus(CustACPlayer player) {
    ...
    private static @Nullable BoatEntityStatus isUnderwater(@NotNull CustACPlayer player) {
        SimpleCollisionBox box = player.boundingBox;
        double maxBoxY = box.maxY + 0.001D;
        int minX = CustACMath.floor(box.minX);
        int maxX = CustACMath.ceil(box.maxX);
        int minY = CustACMath.floor(box.maxY);
        int maxY = CustACMath.ceil(maxBoxY);
        int minZ = CustACMath.floor(box.minZ);
        int maxZ = CustACMath.ceil(box.maxZ);
    ...
    private static boolean checkInWater(CustACPlayer custacPlayer) {
        SimpleCollisionBox box = custacPlayer.boundingBox;
        int minX = CustACMath.floor(box.minX);
        int maxX = CustACMath.ceil(box.maxX);
        int minY = CustACMath.floor(box.minY);
        int maxY = CustACMath.ceil(box.minY + 0.001D);
        int minZ = CustACMath.floor(box.minZ);
        int maxZ = CustACMath.ceil(box.maxZ);
        boolean inWater = false;
        custacPlayer.vehicleData.waterLevel = -Double.MAX_VALUE;

        for (int x = minX; x < maxX; ++x) {
            for (int y = minY; y < maxY; ++y) {
                for (int z = minZ; z < maxZ; ++z) {
                    double level = custacPlayer.compensatedWorld.getWaterFluidLevelAt(x, y, z);
                    if (level > 0) {
                        float f = (float) ((float) y + level);
                        custacPlayer.vehicleData.waterLevel = Math.max(f, custacPlayer.vehicleData.waterLevel);
    ...
    public static float getGroundFriction(CustACPlayer player) {
    ...
    private void floatBoat(CustACPlayer player, Vector3dm vector) {
    ...
    public float getWaterLevelAbove(CustACPlayer player) {
    ...
    private void controlBoat(CustACPlayer player, Vector3dm vector, boolean intermediate) {
        float f = 0.0F;
        ...
        vector.add(new Vector3dm(player.trigHandler.sin(CustACMath.radians(-player.yaw)) * f, 0, (double) (player.trigHandler.cos(CustACMath.radians(player.yaw)) * f)));
    }
}
