 ...
 * Someone trying to false custac by using negative levitation effects
...
public class PointThreeEstimator {
    ...
    public PointThreeEstimator(CustACPlayer player) {
        this.player = player;
    }
    ...
    public void handleChangeBlock(int x, int y, int z, WrappedBlockState state) {
        ...
        if (pointThreeBox.isIntersected(new SimpleCollisionBox(x, y, z))) {
            // https://github.com/MWHunter/CustAC/issues/613
    ...
    private double iterateGravity(CustACPlayer player, double y) {
