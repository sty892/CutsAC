package ac.cust.custac.predictionengine;
...
import ac.cust.custac.utils.latency.CompensatedEntities;
...
import ac.cust.custac.utils.nmsutil.CheckIfChunksLoaded;
import ac.cust.custac.utils.nmsutil.Collisions;
import ac.cust.custac.utils.nmsutil.FluidTypeFlowing;
...
public final class PlayerBaseTick {
    ...
    public static boolean canEnterPose(CustACPlayer player, Pose pose, double x, double y, double z) {
        return Collisions.isEmpty(player, getBoundingBoxForPose(player, pose, x, y, z).expand(-1.0E-7D));
    }

    private static SimpleCollisionBox getBoundingBoxForPose(CustACPlayer player, Pose pose, double x, double y, double z) {
    ...
    public static void doBaseTick(CustACPlayer player) {
    ...
    private static void updateFluidOnEyes(CustACPlayer player) {
    ...
    private static void updateInWaterStateAndDoFluidPushing(CustACPlayer player) {
    ...
    public static void updatePowderSnow(CustACPlayer player) {
    ...
    // 1.14
    public static void updatePlayerPose(CustACPlayer player) {
    ...
    // 1.13 and below
    private static void updatePlayerSize(CustACPlayer player) {
        Pose pose;
        if (player.isGliding) {
            pose = Pose.FALL_FLYING;
        } else if (player.isInBed) {
    ...
    private static void updateSwimming(CustACPlayer player) {
    ...
    private static void moveTowardsClosestSpace(CustACPlayer player, double xPosition, double zPosition) {
    ...
    private static void moveTowardsClosestSpaceLegacy(CustACPlayer player, double x, double z) {
        int floorX = CustACMath.floor(x);
        int floorZ = CustACMath.floor(z);
        int floorY = CustACMath.floor(player.lastY + 0.5);
    ...
    // 1.14+
    private static void moveTowardsClosestSpaceModern(CustACPlayer player, double xPosition, double zPosition) {
    ...
    public static void updateInWaterStateAndDoWaterCurrentPushing(CustACPlayer player) {
        final PacketEntity riding = player.compensatedEntities.self.getRiding();
    ...
    private static boolean updateFluidHeightAndDoFluidPushing(CustACPlayer player, FluidTag tag, double multiplier) {
    ...
    private static boolean updateFluidHeightAndDoFluidPushingLegacy(CustACPlayer player, FluidTag tag, double multiplier) {
        SimpleCollisionBox aABB = player.boundingBox.copy().expand(0, -0.4, 0).expand(-0.001);

        int floorX = CustACMath.floor(aABB.minX);
        int ceilX = CustACMath.ceil(aABB.maxX);
        int floorY = CustACMath.floor(aABB.minY);
        int ceilY = CustACMath.ceil(aABB.maxY);
        int floorZ = CustACMath.floor(aABB.minZ);
        int ceilZ = CustACMath.ceil(aABB.maxZ);
    ...
    private static boolean updateFluidHeightAndDoFluidPushingModern(CustACPlayer player, FluidTag tag, double multiplier) {
    ...
    public static boolean updateFluidInteraction(CustACPlayer player) {
    ...
    private static boolean suffocatesAt(CustACPlayer player, int x, int z) {
    ...
    private static boolean clearAbove(CustACPlayer player, int x, int y, int z) {
