...
import ac.cust.custac.predictionengine.PlayerBaseTick;
import ac.cust.custac.predictionengine.predictions.PredictionEngine;
import ac.cust.custac.predictionengine.predictions.PredictionEngineElytra;
...
import ac.cust.custac.utils.nmsutil.EntityTypeTags;
import ac.cust.custac.utils.nmsutil.FluidFallingAdjustedMovement;
import ac.cust.custac.utils.nmsutil.GetBoundingBox;
import ac.cust.custac.utils.nmsutil.MainSupportingBlockPosFinder;
import ac.cust.custac.utils.viaversion.ViaVersionUtil;
import ac.cust.custac.utils.team.EntityPredicates;
import ac.cust.custac.utils.team.EntityTeam;
...
public class MovementTicker {
    public final CustACPlayer player;

    public static void handleEntityCollisions(CustACPlayer player) {
    ...
    public void move(Vector3dm inputVel, Vector3dm collide) {
        ...
        if (player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_18_2)) {
            boolean xAxis = !CustACMath.equal(inputVel.getX(), collide.getX());
            boolean zAxis = !CustACMath.equal(inputVel.getZ(), collide.getZ());
        ...
        if (collide.lengthSquared() <= 1e-7
            ...
            player.addMovementThisTick(new CustACPlayer.Movement(from, to, new Vector3d(inputVel.getX(), inputVel.getY(), inputVel.getZ())));
        }
    ...
    public void livingEntityTravel() {
        ...
        double lavaLevel = 0;
        if (canStandOnLava())
            lavaLevel = player.compensatedWorld.getLavaFluidLevelAt(CustACMath.floor(player.lastX), CustACMath.floor(player.lastY), CustACMath.floor(player.lastZ));
        ...
        if (player.wasTouchingWater && !player.isFlying) {
            ...
            player.friction = swimFriction; // Not vanilla, just useful for other custac stuff
            ...
            if (player.wasTouchingLava && !player.isFlying && !(lavaLevel > 0 && canStandOnLava())) {
                player.friction = 0.5F; // Not vanilla, just useful for other custac stuff
                ...
                if (player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_21_5) && Collisions.onClimbable(player, player.lastX, player.lastY, player.lastZ)) {
                    ...
                    player.friction = 0.99F; // Not vanilla, just useful for other custac stuff
