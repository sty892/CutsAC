...
import ac.cust.custac.checks.impl.misc.ClientBrand;
import ac.cust.custac.checks.impl.misc.TransactionOrder;
import ac.cust.custac.checks.impl.packetorder.PacketOrderProcessor;
import ac.cust.custac.events.packets.CheckManagerListener;
import ac.cust.custac.manager.*;
import ac.cust.custac.manager.player.features.FeatureManagerImpl;
import ac.cust.custac.manager.player.handlers.DefaultResyncHandler;
import ac.cust.custac.manager.player.handlers.NoOpResyncHandler;
import ac.cust.custac.platform.api.player.PlatformPlayer;
import ac.cust.custac.predictionengine.EntityFluidInteraction;
import ac.cust.custac.predictionengine.MovementCheckRunner;
import ac.cust.custac.predictionengine.PointThreeEstimator;
import ac.cust.custac.predictionengine.UncertaintyHandler;
import ac.cust.custac.manager.AttackCooldownHandler;
...
import ac.cust.custac.utils.change.PlayerBlockHistory;
import ac.cust.custac.utils.collisions.datatypes.SimpleCollisionBox;
import ac.cust.custac.utils.data.*;
import ac.cust.custac.utils.data.packetentity.PacketEntity;
import ac.cust.custac.utils.data.packetentity.PacketEntityHappyGhast;
import ac.cust.custac.utils.data.packetentity.PacketEntityNautilus;
import ac.cust.custac.utils.data.packetentity.PacketEntitySelf;
...
import ac.cust.custac.utils.nmsutil.Materials;
...
public class CustACPlayer implements GrimUser {
    ...
    @Getter @Setter private ResyncHandler resyncHandler = CustACAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("disable-default-resync-handler", false) ? NoOpResyncHandler.INSTANCE : new DefaultResyncHandler(this);
    ...
    // This variable is for support with test servers that want to be able to disable custac
    // CustAC disabler 2022 still working!
    public boolean disableCustAC = false;
    ...
    public CustACPlayer(@NotNull User user) {
        this.user = user;
    ...
    public void onPacketCancel() {
        ...
        if (spamThreshold != -1 && cancelledPackets.incrementAndGet() > spamThreshold) {
            ...
            disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(this, CustACAPI.INSTANCE.getConfigManager().getDisconnectClosed())));
    ...
    public void sendTransaction(boolean async) {
        ...
        if (disableCustAC && (System.nanoTime() - getPlayerClockAtLeast()) > 15e9) {
            return;
        }
    ...
    public void timedOut() {
        disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(this, CustACAPI.INSTANCE.getConfigManager().getDisconnectTimeout())));
    }
    ...
    public void disconnect(Component reason) {
        ...
        if (platformPlayer != null) {
            CustACAPI.INSTANCE.getScheduler().getEntityScheduler().execute(platformPlayer, CustACAPI.INSTANCE.getGrimPlugin(),
    ...
    public void pollData() {
        ...
        if (!CustACAPI.INSTANCE.getPlayerDataManager().shouldCheck(user)) {
            CustACAPI.INSTANCE.getPlayerDataManager().remove(user);
        }
        ...
        if (uuid != null && this.platformPlayer == null) {
            this.platformPlayer = CustACAPI.INSTANCE.getPlatformPlayerFactory().getFromUUID(uuid);
            updatePermissions();
        }
    }
    ...
    public void updatePermissions() {
        ...
        try {
            CustACAPI.INSTANCE.getScheduler().getEntityScheduler().execute(platformPlayer, CustACAPI.INSTANCE.getGrimPlugin(), () -> {
                this.noModifyPacketPermission = platformPlayer.hasPermission("custac.nomodifypacket");
                this.noSetbackPermission = platformPlayer.hasPermission("custac.nosetback");
    ...
    public int getTransactionPing() {
        return CustACMath.floor(transactionPing / 1e6);
    }
    ...
    public final void reload(ConfigManager config) {
        ...
        maxTransactionTime = CustACMath.clamp(config.getIntElse("max-transaction-time", 60), 1, 180);
    ...
    public void resyncPositions(SimpleCollisionBox box) {
        this.resyncHandler.resync(CustACMath.floor(box.minX), CustACMath.floor(box.minY), CustACMath.floor(box.minZ),
                CustACMath.ceil(box.maxX), CustACMath.ceil(box.maxY), CustACMath.ceil(box.maxZ));
    }

    public void addMovementThisTick(CustACPlayer.Movement movement) {
        if (this.movementThisTick.size() >= 100) {
            CustACPlayer.Movement movement1 = this.movementThisTick.removeFirst();
            CustACPlayer.Movement movement2 = this.movementThisTick.removeFirst();
            CustACPlayer.Movement movement3 = new CustACPlayer.Movement(movement1.from(), movement2.to());
