...
import ac.cust.custac.utils.nmsutil.BlockBreakSpeed;
...
public class FastBreak extends Check implements BlockBreakCheck {
    ...
    public FastBreak(CustACPlayer playerData) {
        super(playerData);
    }
    ...
    private void clampBalance() {
        ...
        blockBreakBalance = CustACMath.clamp(blockBreakBalance, -balance, balance); // Clamp not Math.max in case other logic changes
        blockDelayBalance = CustACMath.clamp(blockDelayBalance, -balance, balance);
    }
}
