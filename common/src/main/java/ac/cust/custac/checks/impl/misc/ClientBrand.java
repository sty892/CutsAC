...
public class ClientBrand extends Check implements PacketCheck {
    ...
    public ClientBrand(CustACPlayer player) {
        super(player);
    }
    ...
    private void handle(String channel, byte[] data) {
        ...
        if (data.length > 64 || data.length == 0) {
            ...
            if (!CustACAPI.INSTANCE.getConfigManager().isIgnoredClient(brand)) {
                String message = CustACAPI.INSTANCE.getConfigManager().getConfig().getStringElse("client-brand-format", "%prefix% &f%player% joined using %brand%");
                Component component = MessageUtil.replacePlaceholders(player, MessageUtil.miniMessage(message));

                CustACAPI.INSTANCE.getAlertManager().sendBrand(component, null);
            }
        }
        ...
        if (hasReachHacks && CustACAPI.INSTANCE.getConfigManager().isBlockBlacklistedForgeClients()) {
            player.disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(player, CustACAPI.INSTANCE.getConfigManager().getDisconnectBlacklistedForge())));
        }
