...
import ac.cust.custac.utils.data.packetentity.PacketEntitySizeable;
import ac.cust.custac.utils.data.packetentity.dragon.PacketEntityEnderDragonPart;
...
public class Reach extends Check implements PacketCheck {
    ...
    public Reach(CustACPlayer player) {
        super(player);
    }

    @Override
    public void onPacketReceive(final PacketReceiveEvent event) {
        if (!player.disableCustAC && event.getPacketType() == PacketType.Play.Client.ATTACK) {
        ...
        if (!player.disableCustAC && event.getPacketType() == PacketType.Play.Client.INTERACT_ENTITY) {
    ...
    private void onInteract(PacketReceiveEvent event, int entityId, InteractionHand hand) {
        ...
        // Dead entities cause false flags (https://github.com/CustACAnticheat/CustAC/issues/546)
