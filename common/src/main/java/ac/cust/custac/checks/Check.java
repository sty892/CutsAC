...
@Getter
public class Check extends CustACProcessor implements AbstractCheck {
    protected @NotNull final CustACPlayer player;
    ...
    public Check(final @NotNull CustACPlayer player) {
    ...
    public final void updatePermissions() {
        ...
        exemptPermission = player.platformPlayer.hasPermission("custac.exempt." + id);
        noSetbackPermission = player.platformPlayer.hasPermission("custac.nosetback." + id);
        noModifyPacketPermission = player.platformPlayer.hasPermission("custac.nomodifypacket." + id);
    }
    ...
    public final boolean flag(String verbose) {
        if (player.disableCustAC || (experimental && !player.isExperimentalChecks()) || exemptPermission)
            return false; // Avoid calling event if disabled

        FlagEvent event = new FlagEvent(player, this, verbose);
        CustACAPI.INSTANCE.getEventBus().post(event);
