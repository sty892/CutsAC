...
public class LatencyUtils {
    ...
    public LatencyUtils(CustACPlayer player) {
        this.player = player;
    }
        ...
        synchronized (this) {
            ...
            for (Runnable runnable : tasksToRun) {
                try {
                    ...
                    if (CommonCustACArguments.KICK_ON_TRANSACTION_ERRORS.value()) {
                        player.disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(player, CustACAPI.INSTANCE.getConfigManager().getDisconnectPacketError())));
                    }
                }
            }
        }
