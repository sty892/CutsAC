...
import ac.cust.custac.events.packets.PacketWorldBorder;
import ac.cust.custac.player.CustACPlayer;
import ac.cust.custac.predictionengine.blockeffects.BlockEffectsResolver;
import ac.cust.custac.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_10;
import ac.cust.custac.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_2;
import ac.cust.custac.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_4;
import ac.cust.custac.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_5;
import ac.cust.custac.predictionengine.blockeffects.impl.BlockEffectsResolverV1_21_6;
import ac.cust.custac.utils.chunks.Column;
...
import ac.cust.custac.utils.data.tags.SyncedTags;
...
public final class Collisions {
    ...
    public static boolean slowCouldPointThreeHitGround(CustACPlayer player, double x, double y, double z) {
    ...
    public static Vector3dm collide(CustACPlayer player, double desiredX, double desiredY, double desiredZ) {
        return collide(player, desiredX, desiredY, desiredZ, desiredY, null);
    }

    public static Vector3dm collide(CustACPlayer player, double desiredX, double desiredY, double desiredZ, double clientVelY, VectorData data) {
    ...
    public static boolean addWorldBorder(CustACPlayer player, SimpleCollisionBox wantedBB, List<SimpleCollisionBox> listOfBlocks, boolean onlyCheckCollide) {
    ...
    public static boolean getCollisionBoxes(CustACPlayer player, SimpleCollisionBox wantedBB, List<SimpleCollisionBox> listOfBlocks, boolean onlyCheckCollide) {
    ...
    public static boolean isEmpty(CustACPlayer player, SimpleCollisionBox playerBB) {
    ...
    public static Vector3dm maybeBackOffFromEdge(Vector3dm vec3, CustACPlayer player, boolean overrideVersion) {
    ...
    public static boolean isAboveGround(CustACPlayer player) {
    ...
    public static void handleInsideBlocks(CustACPlayer player) {
    ...
    public static void onInsideBlock(CustACPlayer player, StateType blockType, WrappedBlockState block, int blockX, int blockY, int blockZ, boolean magic) {
    ...
    public static void applyEffectsFromBlocks(CustACPlayer player) {
        ...
        if (clientVersion.isOlderThan(ClientVersion.V_1_21_5)) {
            player.finalMovementsThisTick.add(new CustACPlayer.Movement(from, to));
            ...
            if (player.finalMovementsThisTick.isEmpty()) {
                ...
                player.finalMovementsThisTick.add(new CustACPlayer.Movement(player.finalMovementsThisTick.get(player.finalMovementsThisTick.size() - 1).to(), to));
            }
        }
    ...
    public static void resolveBlockEffects(CustACPlayer player, Vector3d from, Vector3d to) {
        Collisions.resolveBlockEffects(player, List.of(new CustACPlayer.Movement(from, to)));
    }

    public static void resolveBlockEffects(CustACPlayer player, List<CustACPlayer.Movement> movements) {
    ...
    private static double getOldDeltaY(CustACPlayer player, double value) {
        return player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_21_2) ? value / 0.98F + 0.08 : value;
    }

    private static double getNewDeltaY(CustACPlayer player, double value) {
        return player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_21_2) ? (value - 0.08) * 0.98F : value;
    }

    private static boolean isSlidingDown(Vector3dm vector, CustACPlayer player, int locationX, int locationY,
                                         int locationZ) {
        if (player.onGround) {
            return false;
    ...
    // 0.03 hack
    public static boolean checkStuckSpeed(CustACPlayer player, double expand) {
        // Use the bounding box for after the player's movement is applied
    ...
    public static boolean suffocatesAt(CustACPlayer player, SimpleCollisionBox playerBB) {
    ...
    public static boolean doesBlockSuffocate(CustACPlayer player, int x, int y, int z) {
        WrappedBlockState data = player.compensatedWorld.getBlock(x, y, z);
    ...
    // Thanks Tuinity
    public static boolean hasMaterial(CustACPlayer player, SimpleCollisionBox checkBox, Predicate<Pair<WrappedBlockState, Vector3i>> searchingFor) {
    ...
    // Thanks Tuinity
    public static void forEachCollisionBox(@NotNull CustACPlayer player, @NotNull SimpleCollisionBox checkBox, @NotNull BiConsumer<WrappedBlockState, Vector3i> searchingFor) {
    ...
    public static boolean onClimbable(CustACPlayer player, double x, double y, double z) {
    ...
    public static boolean trapdoorUsableAsLadder(CustACPlayer player, double x, double y, double z, WrappedBlockState blockData) {
