...
import ac.cust.custac.utils.data.BlockPrediction;
import ac.cust.custac.utils.data.Pair;
import ac.cust.custac.utils.data.PistonData;
import ac.cust.custac.utils.data.ShulkerData;
import ac.cust.custac.utils.data.packetentity.PacketEntity;
import ac.cust.custac.utils.data.packetentity.PacketEntityShulker;
...
public class CompensatedWorld implements PacketWorld {
    ...
    public CompensatedWorld(CustACPlayer player) {
        this.player = player;
    ...
    public void stopPredicting(PacketWrapper<?> wrapper) {
        ...
        if (PacketEvents.getAPI().getServerManager().getVersion().isNewerThanOrEquals(ServerVersion.V_1_19)) {
            ...
            CustACAPI.INSTANCE.getScheduler().getGlobalRegionScheduler().run(CustACAPI.INSTANCE.getGrimPlugin(), () -> {
    ...
    public double getWaterFluidLevelAt(double x, double y, double z) {
        return getWaterFluidLevelAt(CustACMath.floor(x), CustACMath.floor(y), CustACMath.floor(z));
    }
