...
public class JumpPower {
    public static void jumpFromGround(@NotNull CustACPlayer player, @NotNull Vector3dm vector) {
        ...
        if (player.isSprinting) {
            float radRotation = CustACMath.radians(player.yaw);
    ...
    public static float getJumpPower(@NotNull CustACPlayer player) {
        return (float) player.compensatedEntities.self.getAttributeValue(Attributes.JUMP_STRENGTH) * getPlayerJumpFactor(player);
    }

    public static float getPlayerJumpFactor(@NotNull CustACPlayer player) {
