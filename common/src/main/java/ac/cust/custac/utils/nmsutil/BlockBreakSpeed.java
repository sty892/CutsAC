...
import ac.cust.custac.utils.data.tags.SyncedTag;
import ac.cust.custac.utils.data.tags.SyncedTags;
import ac.cust.custac.utils.enums.FluidTag;
import ac.cust.custac.utils.inventory.EnchantmentHelper;
...
public class BlockBreakSpeed {
    // temporary hardcode to workaround PE bug https://github.com/retrooper/packetevents/issues/1217; see https://github.com/CustACAnticheat/CustAC/issues/2117
    ...
    public static double getBlockDamage(CustACPlayer player, WrappedBlockState block) {
        ItemStack tool = player.inventory.getHeldItem();
        return getBlockDamage(player, tool, block.getType());
    }

    public static double getBlockDamage(CustACPlayer player, ItemStack tool, StateType block) {
        // GET destroy speed
        ...
        final boolean canHarvest = !block.isRequiresCorrectTool() || toolSpeedData.isCorrectToolForDrop
                // temporary hardcode to workaround PE bug https://github.com/retrooper/packetevents/issues/1217; see https://github.com/CustACAnticheat/CustAC/issues/2091
    ...
    private static float getSpeedMultiplierFromToolData(CustACPlayer player, ItemStack tool, ToolSpeedData data) {
    ...
    private static ToolSpeedData getModernToolSpeedData(CustACPlayer player, ItemStack tool, StateType block) {
    ...
    private static ToolSpeedData getLegacyToolSpeedData(CustACPlayer player, ItemStack tool, StateType block) {
        ItemType toolType = tool.getType();
        float speedMultiplier = 1.0f;
        boolean isCorrectToolForDrop = false;
