...
import ac.cust.custac.checks.impl.sprint.SprintD;
...
public class PacketEntitySelf extends PacketEntity {
    ...
    public PacketEntitySelf(CustACPlayer player) {
        super(player, EntityTypes.PLAYER);
        this.player = player;
    }

    public PacketEntitySelf(CustACPlayer player, PacketEntitySelf old) {
        super(player, EntityTypes.PLAYER);
        this.player = player;
    ...
    protected void initAttributes(CustACPlayer player) {
        ...
        trackAttribute(ValuedAttribute.ranged(Attributes.SNEAKING_SPEED, 0.3, 0, 1)
                .withGetRewriter(value -> {
                    ...
                    final double clamped = CustACMath.clamp(0.3f + swiftSneak * 0.15f, 0f, 1f);
                    if (player.getClientVersion().isOlderThan(ClientVersion.V_1_21)) {
                        return clamped;
                    }
