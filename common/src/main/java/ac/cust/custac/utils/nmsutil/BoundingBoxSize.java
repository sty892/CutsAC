...
import ac.cust.custac.utils.data.packetentity.*;
...
public final class BoundingBoxSize {
    ...
    public static float getWidth(CustACPlayer player, PacketEntity packetEntity) {
    ...
    private static float getWidthMinusBaby(CustACPlayer player, PacketEntity packetEntity) {
    ...
    public static Vector3d getRidingOffsetFromVehicle(PacketEntity entity, CustACPlayer player) {
        ...
        if (entity instanceof PacketEntityTrackXRot xRotEntity) {
            ...
            if (EntityTypes.isTypeInstanceOf(entity.type, EntityTypes.BOAT)) {
                ...
                vec3 = yRot(CustACMath.radians(-xRotEntity.interpYaw) - ((float) Math.PI / 2f), vec3);
                return new Vector3d(x + vec3.x, y + (double) f1, z + vec3.z);
            } else if (entity.type == EntityTypes.LLAMA) {
                float f = player.trigHandler.cos(CustACMath.radians(xRotEntity.interpYaw));
                float f1 = player.trigHandler.sin(CustACMath.radians(xRotEntity.interpYaw));
                ...
                float f = player.trigHandler.sin(CustACMath.radians(xRotEntity.interpYaw));
                float f1 = player.trigHandler.cos(CustACMath.radians(xRotEntity.interpYaw));
    ...
    public static float getHeight(CustACPlayer player, PacketEntity packetEntity) {
    ...
    public static double getPassengerRidingOffset(CustACPlayer player, PacketEntity packetEntity) {
    ...
    private static float getHeightMinusBaby(CustACPlayer player, PacketEntity packetEntity) {
        final EntityType type = packetEntity.type;
    ...
    private static float getBabyScaleFactor(CustACPlayer player, PacketEntity packetEntity) {
        final EntityType type = packetEntity.type;
