...
import ac.cust.custac.utils.collisions.HitboxData;
...
import ac.cust.custac.utils.data.HitData;
...
public class WorldRayTrace {
    public static HitData getNearestBlockHitResult(CustACPlayer player, StateType heldItem, boolean sourcesHaveHitbox, boolean fluidPlacement, boolean itemUsePlacement) {
    ...
    public static HitData traverseBlocks(CustACPlayer player, Vector3d start, Vector3d end, BiFunction<WrappedBlockState, Vector3i, HitData> predicate) {
        // I guess go back by the collision epsilon?
        double endX = CustACMath.lerp(-1.0E-7D, end.x, start.x);
        double endY = CustACMath.lerp(-1.0E-7D, end.y, start.y);
        double endZ = CustACMath.lerp(-1.0E-7D, end.z, start.z);
        double startX = CustACMath.lerp(-1.0E-7D, start.x, end.x);
        double startY = CustACMath.lerp(-1.0E-7D, start.y, end.y);
        double startZ = CustACMath.lerp(-1.0E-7D, start.z, end.z);
        int floorStartX = CustACMath.floor(startX);
        int floorStartY = CustACMath.floor(startY);
        int floorStartZ = CustACMath.floor(startZ);
        ...
        double d12 = posXInverse * (xSign > 0 ? 1.0D - CustACMath.frac(startX) : CustACMath.frac(startX));
        double d13 = posYInverse * (ySign > 0 ? 1.0D - CustACMath.frac(startY) : CustACMath.frac(startY));
        double d14 = posZInverse * (zSign > 0 ? 1.0D - CustACMath.frac(startZ) : CustACMath.frac(startZ));
