...
public class SimpleCollisionBox implements CollisionBox {
    ...
    public Vector3d getCenter() {
        return new Vector3d(CustACMath.lerp(0.5, this.minX, this.maxX), CustACMath.lerp(0.5, this.minY, this.maxY), CustACMath.lerp(0.5, this.minZ, this.maxZ));
    }
    ...
    public static Vector3i containing(double x, double y, double z) {
        return new Vector3i(CustACMath.floor(x), CustACMath.floor(y), CustACMath.floor(z));
    }
    ...
    public static Iterable<Vector3i> betweenCornersInDirection(SimpleCollisionBox boundingBox, Vector3d directionVector) {
        ...
        int minX = CustACMath.floor(min.x);
        int minY = CustACMath.floor(min.y);
        int minZ = CustACMath.floor(min.z);
        Vector3d max = boundingBox.max().toVector3d();
        int maxX = CustACMath.floor(max.x);
        int maxY = CustACMath.floor(max.y);
        int maxZ = CustACMath.floor(max.z);
