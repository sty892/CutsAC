...
import ac.cust.custac.utils.data.webhook.discord.CompiledDiscordTemplate;
...
public class MessageUtil {
    ...
    public @Nullable String replacePlaceholders(@Nullable CustACPlayer player, @Nullable String string, boolean removeFormatting) {
        return replacePlaceholders(player, player == null ? null : player.platformPlayer, string, removeFormatting);
    }

    @Contract("_, null -> null; _, !null -> !null")
    public @Nullable String replacePlaceholders(@Nullable CustACPlayer player, @Nullable String string) {
    ...
    public @Nullable String replacePlaceholders(@Nullable PlatformPlayer player, @Nullable String string) {
        return replacePlaceholders(player == null ? null : CustACAPI.INSTANCE.getPlayerDataManager().getPlayer(player.getUniqueId()), player, string, false);
    }
    ...
    private @Nullable String replacePlaceholders(@Nullable CustACPlayer custacPlayer, @Nullable PlatformPlayer platformPlayer, @Nullable String string, boolean removeFormatting) {
        ...
        if (!matcher.find()) {
            return CustACAPI.INSTANCE.getMessagePlaceHolderManager().replacePlaceholders(platformPlayer, string);
        }

        // Get references to the maps once, outside the loop.
        final Map<String, String> staticReplacements = CustACAPI.INSTANCE.getExternalAPI().getStaticReplacements();
        final Map<String, Function<GrimUser, String>> variableReplacements = CustACAPI.INSTANCE.getExternalAPI().getVariableReplacements();
        ...
        String custacReplaced = sb.toString();

        return CustACAPI.INSTANCE.getMessagePlaceHolderManager().replacePlaceholders(platformPlayer, custacReplaced).replace(PLACEHOLDER_ESCAPE_CHAR, '%');
    }

    public @NotNull Component replacePlaceholders(@NotNull CustACPlayer player, @NotNull Component component) {
    ...
    public @NotNull Component miniMessage(@NotNull String string) {
        string = string.replace("%prefix%", CustACAPI.INSTANCE.getConfigManager().getConfig().getStringElse("prefix", "&bCustAC &8»"));
    ...
    public Component getParsedComponent(Sender sender, String key, String fallbackText) {
        String message = CustACAPI.INSTANCE.getConfigManager().getConfig().getStringElse(key, fallbackText);
