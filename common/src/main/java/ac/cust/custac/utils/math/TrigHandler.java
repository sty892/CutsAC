...
public class TrigHandler {
    ...
    public TrigHandler(CustACPlayer player) {
        this.player = player;
    }
    ...
    public Vector3dm getVanillaMathMovement(Vector3dm wantedMovement, float f, float f2) {
        float f3 = VanillaMath.sin(CustACMath.radians(f2));
        float f4 = VanillaMath.cos(CustACMath.radians(f2));
    ...
    public Vector3dm getShitMathMovement(Vector3dm wantedMovement, float f, float f2) {
        float f3 = player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_8) ? OptifineFastMath.sin(CustACMath.radians(f2)) : LegacyFastMath.sin(CustACMath.radians(f2));
        float f4 = player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_8) ? OptifineFastMath.cos(CustACMath.radians(f2)) : LegacyFastMath.cos(CustACMath.radians(f2));
