...
import ac.cust.custac.utils.blockstate.helper.BlockFaceHelper;
...
import ac.cust.custac.utils.nmsutil.Dripstone;
...
public enum BlockPlaceResult {
    ...
    TORCH_OR_HEAD((player, place) -> {
        // type doesn't matter to custac, same hitbox.
    ...
    // As long as one of these properties is met, it is good enough for custac!
    VINE((player, place) -> {
    ...
    SCAFFOLDING((player, place) -> {
        ...
        } // else, cancel if the scaffolding is exactly 7 away, custac doesn't handle this edge case yet.
