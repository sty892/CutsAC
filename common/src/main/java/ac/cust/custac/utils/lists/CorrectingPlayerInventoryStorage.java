package ac.cust.custac.utils.lists;
 ...
 * CustAC uses a hybrid system for inventories - we lag compensate but rely on the server
...
public class CorrectingPlayerInventoryStorage extends InventoryStorage {
    ...
    public CorrectingPlayerInventoryStorage(CustACPlayer player, int size) {
        super(size);
        this.player = player;
    }
    ...
    public void handleClientClaimedSlotSet(int slotID) {
        if (slotID >= 0 && slotID <= Inventory.ITEMS_END) {
            pendingFinalizedSlot.put(slotID, CustACAPI.INSTANCE.getTickManager().currentTick + 5);
        }
    }
    ...
    public void setItem(int item, ItemStack stack) {
        ...
        if (finalTransaction == -1 || player.lastTransactionReceived.get() >= finalTransaction) {
            ...
            pendingFinalizedSlot.put(item, CustACAPI.INSTANCE.getTickManager().currentTick + 5);
    ...
    public void tickWithBukkit() {
        ...
        int tickID = CustACAPI.INSTANCE.getTickManager().currentTick;
        ...
        // If the player's inventory needs to be resent so that CustAC can enable the player's packet inventory again
