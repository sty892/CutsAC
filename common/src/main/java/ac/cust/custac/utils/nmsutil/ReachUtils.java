...
public class ReachUtils {
    ...
    @Contract("_, _, _ -> new")
    public static @NotNull Vector3dm getLook(@NotNull CustACPlayer player, float yaw, float pitch) {
        if (player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_12_2)) {
            float yawRadians = CustACMath.radians(-yaw) - (float) Math.PI;
            float pitchRadians = CustACMath.radians(-pitch);
            ...
            float pitchRadians = CustACMath.radians(pitch);
            float yawRadians = CustACMath.radians(-yaw);
            float pitchCos = player.trigHandler.cos(pitchRadians);
            float x = player.trigHandler.sin(yawRadians);
            float y = player.trigHandler.sin(pitchRadians);
            float z = player.trigHandler.cos(yawRadians);
    ...
    public static double getMinReachToBox(@NotNull CustACPlayer player, @NotNull SimpleCollisionBox targetBox) {
