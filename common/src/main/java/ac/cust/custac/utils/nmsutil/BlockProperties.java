...
import ac.cust.custac.utils.data.MainSupportingBlockData;
...
import ac.cust.custac.utils.data.packetentity.PacketEntityStrider;
...
public class BlockProperties {
    public static float getFrictionInfluencedSpeed(float f, CustACPlayer player) {
     ...
     */
    public static StateType getOnPos(CustACPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
    ...
    public static float getFriction(CustACPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
        if (player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_19_4)) {
    ...
    public static float getBlockSpeedFactor(CustACPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
    ...
    public static boolean onHoneyBlock(CustACPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
     ...
     */
    private static StateType getBlockPosBelowThatAffectsMyMovement(CustACPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
        Vector3i pos = player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_19_4)
                ? new Vector3i(CustACMath.floor(playerPos.getX()), CustACMath.floor(playerPos.getY() - 0.5000001), CustACMath.floor(playerPos.getZ()))
                : getOnPos(player, playerPos, mainSupportingBlockData, 0.500001F);
        return player.compensatedWorld.getBlockType(pos.x, pos.y, pos.z);
    }

    private static Vector3i getOnPos(CustACPlayer player, Vector3d playerPos, MainSupportingBlockData mainSupportingBlockData, float searchBelowPlayer) {
        ...
        if (mainBlockPos != null) {
            ...
            return shouldReturn ? mainBlockPos.withY(CustACMath.floor(playerPos.getY() - (double) searchBelowPlayer)) : mainBlockPos;
        } else {
            return new Vector3i(CustACMath.floor(playerPos.getX()), CustACMath.floor(playerPos.getY() - searchBelowPlayer), CustACMath.floor(playerPos.getZ()));
        }
    }

    public static float getMaterialFriction(CustACPlayer player, StateType material) {
        float friction = 0.6f;
    ...
    private static StateType getOnBlock(CustACPlayer player, double x, double y, double z) {
        StateType block1 = player.compensatedWorld.getBlockType(CustACMath.floor(x), CustACMath.floor(y - 0.2F), CustACMath.floor(z));

        if (block1.isAir()) {
            StateType block2 = player.compensatedWorld.getBlockType(CustACMath.floor(x), CustACMath.floor(y - 1.2F), CustACMath.floor(z));
    ...
    private static float getBlockSpeedFactorLegacy(CustACPlayer player, Vector3d pos) {
    ...
    private static float getBlockSpeedFactor(CustACPlayer player, StateType type) {
    ...
    private static float getModernVelocityMultiplier(CustACPlayer player, float blockSpeedFactor) {
        if (player.getClientVersion().isOlderThan(ClientVersion.V_1_21)) return blockSpeedFactor;
        return (float) CustACMath.lerp((float) player.compensatedEntities.self.getAttributeValue(Attributes.MOVEMENT_EFFICIENCY), blockSpeedFactor, 1.0F);
    }
}
