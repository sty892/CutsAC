...
public class FluidTypeFlowing {
    public static Vector3dm getFlow(CustACPlayer player, int originalX, int originalY, int originalZ) {
        ...
        for (BlockFace enumdirection : new BlockFace[]{BlockFace.NORTH, BlockFace.EAST, BlockFace.SOUTH, BlockFace.WEST}) {
            ...
            if (affectsFlow(player, originalX, originalY, originalZ, modifiedX, originalY, modifiedZ)) {
                ...
                if (f == 0.0F) {
                    ...
                    // CustAC's definition of solid is whether the block has a hitbox
    ...
    private static boolean affectsFlow(CustACPlayer player, int originalX, int originalY, int originalZ, int x2, int y2, int z2) {
        return isEmpty(player, x2, y2, z2) || isSame(player, originalX, originalY, originalZ, x2, y2, z2);
    }

    private static boolean isSolidFace(CustACPlayer player, int originalX, int y, int originalZ, BlockFace direction) {
    ...
    public static boolean isEmpty(CustACPlayer player, int x, int y, int z) {
    ...
    public static boolean isSame(CustACPlayer player, int x1, int y1, int z1, int x2, int y2, int z2) {
