...
public class VectorUtils {
    ...
    public static @NotNull Vector3dm cutBoxToVector(double x, double y, double z, @NotNull SimpleCollisionBox box) {
        return new Vector3dm(CustACMath.clamp(x, box.minX, box.maxX),
                CustACMath.clamp(y, box.minY, box.maxY),
                CustACMath.clamp(z, box.minZ, box.maxZ));
    }
    ...
    public static @NotNull Vector3d clampVector(@NotNull Vector3d toClamp) {
        double x = CustACMath.clamp(toClamp.getX(), -3.0E7D, 3.0E7D);
        double y = CustACMath.clamp(toClamp.getY(), -2.0E7D, 2.0E7D);
        double z = CustACMath.clamp(toClamp.getZ(), -3.0E7D, 3.0E7D);
